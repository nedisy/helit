...
# This version has a fatter compared to the normal one, so you get a larger overlap region.
...
def makeDoc():
  ...
  for i in xrange(docWordCount):
    if random.random()<0.0: # Increase to try with noise.
      ...
      key = ((1+random.randrange(2))*5 + random.randrange(5), 0)
    else:
      key = (random.randrange(5)*5 + (1+random.randrange(2)), 0)
...
cv.SaveImage('test_cross_fat_docs.png',img)
...
for r in xrange(c.getRegionCount()):
  ...
  img = array2cv(data)
  cv.SaveImage('test_cross_fat_region_%i.png'%r,img)
...
for t,data in enumerate(topicRendList):
  ...
  data = numpy.repeat(numpy.repeat(data,25,axis=0),25,axis=1)
  img = array2cv(data)
  cv.SaveImage('test_cross_fat_topic_%i.png'%t,img)
